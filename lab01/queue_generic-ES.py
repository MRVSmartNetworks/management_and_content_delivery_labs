...
def departure(time, FES, queue, serv_id, serv, n_server):
    ...
    if len(queue) > 0:
        ...
        if n_server is not None:
            ...
            data.serv_busy[serv_id]["cumulative_time"] += (
                time - data.serv_busy[serv_id]["begin_last_service"]
            )
...
if __name__ == "__main__":
    ...
    if single_run:
        ...
        arr_t = (
            1.0 / arr_rate
        )  # is the average inter-arrival time; arrival rate = 1/ARRIVAL

        if isinstance(serv_rate, int) or isinstance(serv_rate, float):
            serv_t = (
                1.0 / serv_rate
            )  # is the average service time; service rate = 1/SERVICE
        elif isinstance(serv_rate, list):
            serv_t = [1.0 / x for x in serv_rate]

        if n_server is not None and (
            isinstance(serv_t, int) or isinstance(serv_t, float)
        ):
        ...
        if queue_len is not None:
            if n_server is not None:
                queue_len = max(
                    queue_len, n_server
                )  # This ensure compatible parameters
            else:
                queue_len = None
        ########################################
    ...
    if change_arr_t:
        ...
        for arr_t in arr_t_list:
            MM_system, data, time = run(
                arr_t=arr_t, serv_t=serv_t, n_server=n_server, queue_len=queue_len
            )
            ...
            if confidence_int:
                ...
                intervals.append(
                    t.interval(
                        ...
                        n_iter - 1,
                        np.mean(metric_cf),
                        np.std(metric_cf) / np.sqrt(n_iter),
                    )
                )
        ...
        if confidence_int:
            ...
            metric = "avg_delay"
            plotConfInter(
                metric, n_iter, conf_level, intervals, metric_cf_mean, arr_t_list
            )
    ...
    if multi_vs_single:
        ...
        MM1, data1, time1 = run(
            arr_t=arr_t, serv_t=serv_t, n_server=1, queue_len=queue_len
        )  # MM1 system
        MM2, data2, time2 = run(
            arr_t=arr_t, serv_t=serv_t, n_server=2, queue_len=queue_len
        )  # MM2 system
        printResults(
            1,
            "first-idle",
